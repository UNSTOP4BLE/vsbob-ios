...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		trace('INFORMATION test trace lol\n');
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'sunshine' :
			{
				var bg:FlxSprite = new FlxSprite(-400).loadGraphic(Paths.image('bob/bg/happy'));
				bg.scale.set(2.2,2);
				bg.updateHitbox();
				bg.active = false;
				add(bg);
			...
			case 'withered' :
			{
				var bg:FlxSprite = new FlxSprite(-400).loadGraphic(Paths.image('bob/bg/annoyed'));
				bg.scale.set(2.2,2);
				bg.updateHitbox();
				bg.active = false;
				add(bg);
			...
			case 'run' | 'run-remix-because-its-cool' :
			{
				...
				{
					var bg:FlxSprite = new FlxSprite( -400).loadGraphic(Paths.image('bob/bg/happyhell'));
					bg.scale.set(2.2, 2);
					bg.updateHitbox();
					bg.active = false;
					add(bg);
				...
				{
					var bg:FlxSprite = new FlxSprite( -400).loadGraphic(Paths.image('bob/bg/hell'));
					bg.scale.set(2.2, 2);
					bg.updateHitbox();
					bg.active = false;
					add(bg);
			...
			case 'onslaught' :
			{
				defaultCamZoom = 0.9;
				curStage = 'slaught';
				var bg:FlxSprite = new FlxSprite( -400).loadGraphic(Paths.image('bob/bg/scary'));
				bg.scale.set(2.2, 2);
				bg.updateHitbox();
				bg.active = false;
				add(bg);
			...
			case 'trouble' :
			{
				...
				var bg:FlxSprite = new FlxSprite(-410,-250).loadGraphic(Paths.image('bob/bg/unhappy'));
			...
			case 'ron' | 'little-man':
			{
				...
				var bg:FlxSprite = new FlxSprite(-410,-10).loadGraphic(Paths.image('bob/bg/ron'));
				bg.scale.set(2.6,2.4);
				bg.updateHitbox();
				bg.active = false;
				add(bg);
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 30,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - KE " + MainMenuState.kadeEngineVer + " - " + (FlxG.save.data.etternaMode ? "E.Mode" : "FNF"), 16);
		kadeEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		kadeEngineWatermark.scrollFactor.set();
		add(kadeEngineWatermark);
		var iosWatermark = new FlxText(4,healthBarBG.y + 45,0, "Por" + "t" + " by " + "UN" + "STO" +"P4B" + "LE", 16);
		iosWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		iosWatermark.scrollFactor.set();
		add(iosWatermark);

		if (FlxG.save.data.downscroll)
		{
			kadeEngineWatermark.y = FlxG.height * 0.9 + 25;
			iosWatermark.y = FlxG.height * 0.9 + 40;
		}
		...
		_pad = new FlxVirtualPad(NONE, Pause);
		_pad.alpha = 0.75;
	...
	override public function update(elapsed:Float)
	{
		...
		if ((FlxG.keys.justPressed.ENTER || _pad.buttonPause.justPressed) && startedCountdown && canPause)
		{
