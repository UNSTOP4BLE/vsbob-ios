...
class IosControlsSubState extends FlxSubState
{
	...
	var curSelected:Int = IosControls.getMode();
	...
	override function create()
	{
		...
		funitext = new FlxText(0, 50, 0, 'No Ios Controls!', 32);
		...
		leftArrow.frames = FlxAtlasFrames.fromSparrow(Assets.getBitmapData('assets/ios/menu/arrows.png'),
			Assets.getText('assets/ios/menu/arrows.xml'));
		leftArrow.animation.addByPrefix('idle', 'arrow left');
		leftArrow.animation.play('idle');
		add(leftArrow);

		rightArrow = new FlxSprite(inputvari.x + inputvari.width + 10, inputvari.y - 25);
		rightArrow.frames = FlxAtlasFrames.fromSparrow(Assets.getBitmapData('assets/ios/menu/arrows.png'),
			Assets.getText('assets/ios/menu/arrows.xml'));
		rightArrow.animation.addByPrefix('idle', 'arrow right');
		rightArrow.animation.play('idle');
		add(rightArrow);

		var tipText:FlxText = new FlxText(10, FlxG.height - 24, 0, 'Press B to get back to the options menu', 16);
	...
	override function update(elapsed:Float)
	{
		...
		{
			IosControls.setMode(curSelected);

			if (controlsItems[Math.floor(curSelected)] == 'Pad-Custom')
				IosControls.setCustomMode(virtualPad);
