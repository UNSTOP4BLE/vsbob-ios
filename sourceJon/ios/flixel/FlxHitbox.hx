...
class FlxHitbox extends FlxSpriteGroup
{
	...
	private function createHint(X:Float, Y:Float, Graphic:String, Color:Int = 0xFFFFFF):FlxButton
	{
		...
		hint.loadGraphic(FlxGraphic.fromFrame(FlxAtlasFrames.fromSparrow(Assets.getBitmapData('assets/ios/hitbox.png'),
			Assets.getText('assets/ios/hitbox.xml'))
			.getByName(Graphic)));
		...
		hint.onDown.callback = function()
		{
			...
			hintTween = FlxTween.tween(hint, {alpha: IosControls.getOpacity()}, 0.01, {ease: FlxEase.circInOut, onComplete: function(twn:FlxTween)
			{
				hintTween = null;
			}});
		}
